...
public class GameManager : MonoBehaviour
{
    ...
    [Tooltip("Call controller button event.")]
    ...
    private void Start()
    {
        ...
        #region Right controller button event
        // Right controller A key
        XRControllerManager.Instance.BingingPrimaryHotKey(false, (args) =>
        {
            Debug.Log("Press the A key on the right controller");

            switch (_state)
            {
                case ExperienceState.WaitingForSelection:
                    SelectModelAndStart(useLucy: false); // Mirage
                    break;

                case ExperienceState.Running:
                    ShowWave();
                    webRTCController.ControllerEvent("A");
                    break;
                default:
        ...
        // Right controller B key
        XRControllerManager.Instance.BingingSecondaryHotKey(false, (args) =>
        {
            Debug.Log("Press the B key on the right controller");

            switch (_state)
            {
                case ExperienceState.WaitingForSelection:
                    SelectModelAndStart(useLucy: true); // Lucy
                    break;

                case ExperienceState.Running:
                    ShowWave();
                    webRTCController.ControllerEvent("B");
                    break;
                default:
                    throw new ArgumentOutOfRangeException();
            }
    ...
    private void Update()
    {
        ...
        {
            case ExperienceState.WaitingForSelection:
                if (Input.GetKeyDown(KeyCode.A)) // Mirage
                {
                    SelectModelAndStart(useLucy: false);
                }
                else if (Input.GetKeyDown(KeyCode.B)) // Lucy
                {
